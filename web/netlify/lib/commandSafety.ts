...
const CAMPAIGN_ACTIVE_STATE =
  /(\b(?:campagne|envoi)\s+(?:(?:est|reste|semble)(?:-t-elle|-t-il)?\s+)?(?:actuellement\s+)?)active\b/giu;

const CAMPAIGN_PAUSED_STATE =
  /(\b(?:campagne|envoi)\s+(?:(?:est|reste|semble)(?:-t-elle|-t-il)?\s+)?(?:actuellement\s+)?)en\s+pause\b/giu;
