...
describe("RECHERCHES_HEADERS", () => {
  it("has 11 columns", () => {
    expect(RECHERCHES_HEADERS).toHaveLength(11);
  });

  it("contains all fields", () => {
    expect(RECHERCHES_HEADERS).toEqual(["id", "description", "mode", "filtres_json", "nb_resultats", "date", "user_id", "scoring_status", "scoring_instructions", "scoring_mode", "scoring_criteria_prompt"]);
  });
});
